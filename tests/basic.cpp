...
void quadratic_integrand(builtin_real& holder, builtin_real x) {
	holder = x * x + 3;
//	std::cout << "(" << (double)x << ", " << (double)holder << ")\n";
}

builtin_real fast_exp(builtin_real exponent) {
	if (exponent > 0) {
		return exp(exponent);
	}
	const unsigned int LOG_N = 50;
	const builtin_real ONE = 1.0;
	const builtin_real DENOM = (long long)1 << LOG_N;
	builtin_real x = ONE + exponent / DENOM;
...
void gaussian(builtin_real& holder, builtin_real x) {
	holder = fast_exp(-(x * x));
}

void sqrt_integrand(builtin_real& holder, builtin_real x) {
	holder = sqrt(x);
}

void test_fast_exp() {
	for (builtin_real x = 0.0; x > -10; x -= 0.1) {
...
void test_quadratic() {
	...
	Simpson_Sums<builtin_real> sums{0., 0., 0.};
	//std::cout << std::fixed;
	std::cout.precision(dbl::digits10);
	builtin_real result = simpson_rule<builtin_real>(quadratic_integrand, sp, sums);
	std::cout << "The integral of x^2 + 3 from x = 0 to x = 3 is " << (double)result << "\n";
	builtin_real actual = 18.0;
...
void test_gaussian() {
	...
	//std::cout << std::fixed;
	std::cout.precision(dbl::digits10);
	builtin_real result = simpson_rule<builtin_real>(gaussian, sp, sums);
	std::cout << "The integral of e^(-x^2) from x = -5 to x = 5 is " << (double)result << "\n";
	builtin_real actual = 1.772453850902790950764921109937813548789156376909461997943865086066573890495904715920494455420022794;
	std::cout << "Error:\t" << std::abs((double)(result - actual)) << "\n\n";
}
...
void test_sqrt() {
	...
	Simpson_Sums<builtin_real> sums{0., 0., 0.};
	std::cout.precision(dbl::digits10);
	builtin_real result = simpson_rule<builtin_real>(sqrt_integrand, sp, sums);
	std::cout << "The integral of sqrt(x) from x = 1 to x = 101 is " << (double)result << "\n";
	builtin_real actual = 676.0249584888066115280971707924781299209649136484427375652;
	std::cout << "Error:\t" << std::abs((double)(result - actual)) << "\n\n";
}
